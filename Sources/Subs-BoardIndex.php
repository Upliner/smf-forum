...
function getBoardIndex($boardIndexOptions)
{
	...
	while ($row_board = $smcFunc['db_fetch_assoc']($result_boards))
	{
		...
		{
			$this_last_post['href'] = $scripturl . '?topic=' . $row_board['id_topic'] . '.msg' . ($user_info['is_guest'] ? $row_board['id_msg'] : $row_board['new_from']) . (empty($row_board['is_read']) ? ';boardseen' : '') . ';forcetopic#new';
