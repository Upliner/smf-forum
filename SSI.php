...
function ssi_queryPosts($query_where = '', $query_where_params = array(), $query_limit = 10, $query_order = 'm.id_msg DESC', $output_method = 'echo', $limit_body = false, $override_permissions = false)
{
	...
	while ($row = $smcFunc['db_fetch_assoc']($request))
	{
		...
		$posts[] = array(
			...
			'href' => $scripturl . '?topic=' . $row['id_topic'] . '.msg' . $row['id_msg'] . ';forcetopic;topicseen#new',
			'link' => '<a href="' . $scripturl . '?topic=' . $row['id_topic'] . '.msg' . $row['id_msg'] . ';forcetopic#msg' . $row['id_msg'] . '" rel="nofollow">' . $row['subject'] . '</a>',
...
function ssi_recentTopics($num_recent = 8, $exclude_boards = null, $include_boards = null, $output_method = 'echo')
{
	...
	while ($row = $smcFunc['db_fetch_assoc']($request))
	{
		...
		$posts[] = array(
			...
			'link' => '<a href="' . $scripturl . '?topic=' . $row['id_topic'] . '.msg' . $row['id_msg'] . ';forcetopic#new" rel="nofollow">' . $row['subject'] . '</a>',
	...
	while ($row = $smcFunc['db_fetch_assoc']($request))
	{
		...
		$attachments[$row['id_attach']] = array(
			...
			'topic' => array(
				'id' => $row['id_topic'],
				'subject' => $row['subject'],
				'href' => $scripturl . '?topic=' . $row['id_topic'] . '.msg' . $row['id_msg'] . ';forcetopic#msg' . $row['id_msg'],
				'link' => '<a href="' . $scripturl . '?topic=' . $row['id_topic'] . '.msg' . $row['id_msg'] . ';forcetopic#msg' . $row['id_msg'] . '">' . $row['subject'] . '</a>',
				'time' => timeformat($row['poster_time']),
			),
		);
